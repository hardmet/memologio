...
class PostEndpoint[F[_] : Sync, PostId](override val service: PostService[F, PostId],
  ...
  def processSearchResult(result: Either[String, Vector[Post.Existing[PostId]]]): F[Response[F]] =
    result.fold(
      BadRequest(_),
      posts =>
